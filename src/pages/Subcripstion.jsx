...
const Subscription = () => {
  const { user, refreshUserStatus, loading } = useContext(AuthContext);
  ...
  // Refresh Firebase token and fetch fresh Mongo user data on mount or user changes
  useEffect(() => {
  ...
  const { mutate: handleSubscribe } = useMutation({
    ...
    onError: (error) => {
      toast.dismiss();
      const errorMessage =
        error.response?.data?.error || error.message || "Subscription failed. Please try again.";
      toast.error(
        <div className="flex items-center">
  ...
  if (loading || (user && !mongoUser && !initialLoadComplete)) {
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <Helmet>
        ...
        <meta
          name="description"
          content="Choose a subscription plan to unlock premium content."
        />
      </Helmet>
        ...
        {mongoUser?.premiumTaken && (
          <div className="max-w-2xl mx-auto mb-10 bg-gradient-to-r from-green-100 to-teal-100 p-6 rounded-xl shadow-md border border-green-200">
            ...
            <p className="text-center text-green-700 mt-2">
              {new Date(mongoUser.premiumTaken) > new Date()
                ? `Active until ${new Date(mongoUser.premiumTaken).toLocaleString()}`
                : `Expired on ${new Date(mongoUser.premiumTaken).toLocaleDateString()}`}
            </p>
          </div>
        )}
